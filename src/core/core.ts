export class WKTools {
	constructor(elementList) {
		return elementList;
	}
}

export const fn = function(){}.prototype;
...
window['$'] = function(...args) {
	if (typeof args[0] === 'function') {
		...
		return new WKTools(_collection);
...
const selectedElements = collection => {
	...
	collection.on = (eventName: string, handler) => {
		collection.forEach((element) => {
			element.addEventListener(eventName, handler);
		});
		return collection;
	};
	collection.addClass = (_classes: string) => {
	...
	collection.removeClass = (_classes: string) => {
		let classes = _classes.split(" ");
		collection.forEach((element) => {
			classes.forEach((_class) => {
	...
	collection.hasClass = (_class: string) => {
	...
	collection.template = (template: string) => {
		const regex = /{{(.*?)}}/gm;
		collection.forEach((element) => {
